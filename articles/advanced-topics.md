...
After generating mock data, calculate derived variables using your
custom functions:
...
MockData uses unique identifiers to track variables and categories
throughout the metadata lifecycle. UIDs provide traceability and version
control for harmonization workflows.
...
MockData supports generating data for multiple databases/cycles using
the `databaseStart` parameter. This enables testing harmonization code
across survey cycles.

### Database filtering

The `databaseStart` column in `variable_details.csv` specifies which
databases each category applies to:
...
Different databases may have different category codes for the same
variable:
...
MockData automatically filters to the correct rules based on
`databaseStart`.
...
handles this internally, so most users won’t need to worry about
duplicate checking.
...
1.  **Metadata preparation:** Use recodeflow metadata format
    (variables.csv, variable_details.csv)
...
- **Reproducible testing:** Same mock data every time (use seed
  parameter)
- **Edge case testing:** Easy to create extreme scenarios (prop_garbage
  parameter)
- **Documentation:** Mock data examples clarify harmonization logic
- **CI/CD integration:** Automated testing without data access
  restrictions
...
Use different seeds for different variables to ensure independence while
...
Always document the seed values used for data generation in your code
comments or project documentation. This ensures others can reproduce
your exact mock datasets.
...
- Learn about [configuration
  ...
  for complete metadata schema
- Understand [missing data
  handling](https://big-life-lab.github.io/MockData/articles/tutorial-missing-data.md)
  in health surveys
- See [MockData for recodeflow
  users](https://big-life-lab.github.io/MockData/articles/for-recodeflow-users.md)
  for harmonization workflows
...
- [MockData for recodeflow
  users](https://big-life-lab.github.io/MockData/articles/for-recodeflow-users.md) -
  Integration with cchsflow/chmsflow
