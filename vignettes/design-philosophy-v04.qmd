...
Algebraic formula-derived variables landed in v0.5 (Phase A): a variable
carrying a `mockFormula` expression (e.g. `weight / (height^2)`) is generated
by evaluating that expression, in dependency order, over a restricted
sandbox exposing only the generated columns and a fixed allow-list of base
functions. `Func::` dispatch to a consuming package's own derivation
function (Phase B), and a more general evaluator, remain deferred.
