...
stopifnot(
  ...
  # Every event would fall 1,825 days after entry, so a removed event must
  # have a death before that day.
  "every removed event has a death before its event day" =
    all(demo$death[is.na(demo$event)] < demo$entry[is.na(demo$event)] + 1825),
  "no remaining event falls after a death" =
    sum(demo$death < demo$event, na.rm = TRUE) == 0
)
```

Everyone would have the event (`event_prop = 1`) exactly five years after entry, but `r n_censored` of 1,000 events are removed because death came first.
