...
Survival dates (rows that set `anchor`, from v0.5) are generated only by the
v0.4 pipeline. If one of these conditions selects the legacy path for metadata
that sets `anchor`, `create_mock_data()` stops and names the survival
variables rather than dropping them.
