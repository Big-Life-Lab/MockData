...
- **Old interview dates (≤ 1990)**: `r n_old` (`r old_pct`%)
...
Add garbage to individual survival date variables with `add_garbage()`, then generate them with `create_mock_data()`:
...
- **Add temporal violations in survival data** by adding garbage to individual survival date variables
