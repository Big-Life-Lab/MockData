...
validate_mock_data_config <- function(config) {
  ...
  # Check required columns (uid is optional for simple examples)
  required_cols <- c("variable", "role", "variableType", "position")
  ...
  # Check unique uid values (if uid column exists)
  if ("uid" %in% names(config) && any(duplicated(config$uid))) {
