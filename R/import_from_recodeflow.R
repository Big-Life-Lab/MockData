...
#' Direct copy: variable, dummyVariable, recStart (to recEnd), catStartLabel (to catLabel),
...
import_from_recodeflow <- function(
  ...
  if (nrow(details_filtered) > 0) {
    ...
    details <- data.frame(
      ...
      recEnd = details_filtered$recStart,  # recStart -> recEnd
  ...
  message("  3. Fill in 'proportion' values in details file (must sum to 1.0 per variable)")
  message("  4. Add any additional distribution parameters (mean, sd, range_min, range_max, etc.)")
