...
.check_simstudy_variable <- function(variable) {
  ...
  if (variable$type == "categorical" && any(grepl(";", as.character(variable$levels), fixed = TRUE))) {
    # simstudy encodes categorical probabilities and labels as semicolon-
    # delimited strings; labels containing ';' cannot round-trip safely.
    stop(
...
.simstudy_definition <- function(def, variable) {
  ...
  if (variable$type == "categorical") {
    ...
    # defData() expects categorical probabilities in formula and labels in
    # variance, both as semicolon-delimited vectors.
    return(simstudy::defData(
      dtDefs = def,
      varname = variable$name,
...
.normalize_simstudy_categorical <- function(values, variable) {
  ...
  # Older simstudy versions returned 1-based category indices in some paths;
  # keep the adapter explicit so cross-version drift cannot masquerade as data.
...
generate_mock_data_simstudy <- function(spec, n, seed = NULL) {
  ...
  .with_mock_seed(seed, {
    ...
    # Normal/date variables stay native until MockData has an explicit contract
    # for mapping their range and calendar semantics into simstudy definitions.
