...
#' harmonization (recodes to NA::b), not a population category for generation.
#'
#' @keywords internal
get_variable_categories <- function(var_details, include_na = FALSE) {
  ...
  # Check if recEnd column exists
  if (!"recEnd" %in% names(var_details)) {
    warning("recEnd column not found in variable_details. ",
            "Cannot distinguish missing codes (NA::a, NA::b) from valid categories. ",
            "Returning empty character vector. ",
            "Add recEnd column to variable_details to enable missing data classification.")
    return(character(0))
  }
  ...
  # IMPORTANT: Exclude "else" rows (garbage collector that recodes to NA::b)
  # "else" is an exception to "recodes to NA::b = missing code" rule
  # This matches the exclusion in extract_proportions() for config-driven generation
