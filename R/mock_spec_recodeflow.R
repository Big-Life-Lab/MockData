...
#' @noRd
# Formula-derived variables (#39, Phase A). A variable's mockFormula is a
# variable-level fact but is stored row-level on variable_details (ADR
# v05-formula-evaluator D1(a)): a variable's details may span multiple rows
# (e.g. one DerivedVar:: row plus missing-code rows), so read the first
# non-blank mockFormula among them. Returns "" when the column is absent or
# every value is blank/NA.
.details_mock_formula <- function(details) {
  if (is.null(details) || nrow(details) == 0 || !"mockFormula" %in% names(details)) {
    return("")
  }

  values <- as.character(details$mockFormula)
  values <- values[!vapply(values, .is_blank, logical(1))]
  if (length(values) == 0) {
    return("")
  }
...
.recodeflow_to_spec_variable <- function(var_row, details, databaseStart) {
  ...
  # (#39, Phase A, ADR D1(a)/D6): a non-blank mockFormula on the variable's
  # detail rows means it is generated by evaluate_mock_formulas(), regardless
  # of what variableType/rType the row otherwise declares. Missing-code and
  # garbage settings are reused as-is so postprocess behaves identically to
  # any other variable.
  mock_formula <- .details_mock_formula(details)
  if (!.is_blank(mock_formula)) {
    return(mock_spec_formula(
      name = variable,
...
mock_spec_from_recodeflow <- function(variables,
  ...
  if (isTRUE(exclude_derived) && !is.null(variable_details)) {
    ...
    # D6 carve-out (#39, Phase A): a DerivedVar::/Func:: variable carrying a
    # non-blank mockFormula is intentionally generated by
    # evaluate_mock_formulas(), not excluded here. Bare DerivedVar::/Func::
    # rows (no mockFormula) keep today's exclude-and-defer-to-recodeflow
    # behaviour unchanged.
