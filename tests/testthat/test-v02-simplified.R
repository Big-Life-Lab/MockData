...
test_that("create_cat_var handles 'else' in recEnd", {
  ...
  expect_s3_class(result$SMK_01, "factor")
  expect_true(all(result$SMK_01 %in% c("1", "2", "3")))
})
...
test_that("determine_proportions extracts from proportion column", {
  ...
  result <- determine_proportions(details_subset)

  expect_type(result, "list")
  expect_equal(length(result), 3)
  expect_equal(sum(unlist(result)), 1.0)
})
...
test_that("get_enabled_variables filters correctly", {
  ...
  result <- get_enabled_variables(config)
