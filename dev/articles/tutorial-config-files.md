# Generating datasets from configuration files

**About this vignette:** This tutorial demonstrates the metadata-driven
approach to batch mock data generation. All examples use executable code
with real metadata files.

## Why configuration files?

MockData takes a **metadata-driven approach** that differs from other
...
**MockData**: Define data structure in metadata files, reuse for both
harmonization and mock data generation
...
- **Reuse existing recodeflow metadata** - No need to duplicate variable
  definitions
- **Single source of truth** - Variable structure defined once, used
  everywhere
- **Consistency with harmonization** - Mock data matches real data
  structure exactly
- **Easy to maintain** - Update metadata once, affects both
  harmonization and mock data
...
**Alternative approach**: For learning or working with just a few
variables, see [Getting
started](https://big-life-lab.github.io/MockData/articles/getting-started.md)
for the variable-by-variable approach.
...
This tutorial will show you how to use these columns step-by-step.
Here’s a quick reference for later:
...
- `"copy"` - Pass through value from recStart (use for ranges like
  “\[18,100\]”)
- Same as recStart - Output specific value (e.g., “1”, “2”, “999”)
- `"corrupt_high"` / `"corrupt_low"` - Generate garbage data in
  specified range
- `"mean"` / `"sd"` - Specify normal distribution parameters (advanced)

## Your first configuration file

Let’s start with the simplest possible example: a single categorical
variable. This shows the basic pattern you’ll use for all variables.
...
- `role`: Set to “enabled” to generate this variable (for dates, use
  role like “baseline-date”)
- `variableType`: “categorical” or “continuous” (dates use
  “continuous” + role for recodeflow compatibility)
- `position`: Order in which variables are generated (1 = first, 2 =
  second, etc.)

**Note on `stringsAsFactors = FALSE`:** This R parameter prevents
automatic conversion of character columns to factors when creating data
frames. Always set this to FALSE to preserve the exact data types you
specify.
...
- `recStart`: The input value or range
  ...
  - For continuous: Ranges like “\[18, 100\]” or missing codes like
    “999”
...
- `proportion`: Optional proportion for this category (must sum to 1.0
  within each variable)
- `rType`: Optional R data type for output (e.g., “integer”, “factor”,
  “Date”)
...
- `details_path`: Path to CSV file specifying variable structure
  (categories, ranges, proportions)
- `n`: Number of observations (rows) to generate
- `seed`: Random seed for reproducibility - same seed always produces
  identical data
- `verbose`: Set to FALSE to suppress informational messages during
  generation (useful for cleaner output in reports)
- `validate`: Optional, defaults to TRUE - validates metadata before
  generation
...
Research papers typically include a “Table 1” with descriptive
statistics. MockData lets you generate data that matches these published
statistics.

**Example scenario**: A paper reports smoking prevalence in their
cohort:
...
**Key insight:** Proportions must sum to 1.0 for each variable. MockData
samples according to these proportions, making it easy to match
...
**Note**: Added `rType` column to specify output types (factor for
smoking, integer for age).
...
- Smoking: ~28% daily, ~18% occasional, ~54% never (close to specified
  proportions)
...
Date variables follow a special pattern for compatibility with
recodeflow metadata:

- `variableType = "continuous"` (dates are stored as numbers in
  recodeflow)
...
**Key insight:** This pattern maintains compatibility with recodeflow
(where dates are `variableType = "continuous"`) while allowing MockData
to generate proper Date objects using `rType`.

## Adding data quality issues (garbage data)

Real datasets have garbage values (data entry errors, out-of-range
values). MockData can simulate these for testing data validation
pipelines.
...
See [Garbage data
documentation](https://big-life-lab.github.io/MockData/articles/cchs-example.html#garbage-data)
for complete specifications.

## Working with existing recodeflow metadata

For real projects, you’ll reuse existing harmonization metadata.
MockData works with the same files used by recodeflow:
...
ADL_01, ADL_02, ADL_03, ADL_04, ADL_05, ADL_06, ADL_07, ADL_der,
ADL_score_5, ADL_score_6

- Total variables: 74
- Details rows: 669

**Key insight:** These are the same metadata files used for
harmonization. By reusing them for mock data generation, you ensure
consistency between mock and real data structures.
...
For reproducible workflows, save your configurations as CSV files. This
makes them version-controllable and shareable:
...
- **Why metadata-driven**: MockData’s unique approach reuses
  harmonization metadata
- **Seeding configs**: Start simple (one variable) then build complexity
- **Table 1 matching**: Use proportions to match published descriptive
  statistics
...
- **Reproducibility**: Save configurations and use seeds for identical
  results
...
- [Getting
  started](https://big-life-lab.github.io/MockData/articles/getting-started.md) -
  Variable-by-variable approach for learning
- [Missing
  data](https://big-life-lab.github.io/MockData/articles/missing-data.md) -
  Detailed missing data patterns
- [Date
...
- [CCHS
  ...
  Canadian Community Health Survey workflow
- [CHMS
  example](https://big-life-lab.github.io/MockData/articles/chms-example.md) -
  Canadian Health Measures Survey workflow
- [DemPoRT
