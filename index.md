...
data from small metadata specifications. Those specifications can be
provided as `variables.csv` and `variable_details.csv` files, or as
simple in-memory data frames when you just want to create one variable
or a small example. It is useful today for some development and
documentation workflows, especially when paired with recodeflow-style
metadata, but it should be treated as experimental infrastructure rather
than a stable released package.
...
- Some error handling is too permissive and can fail with warnings
  instead of stopping
- Some documentation is ahead of the implementation
...
MockData does not use language models to generate data. The data are
generated by R code from metadata and user-specified rules. Some package
development has used AI coding tools, with human review; we do not
credit a specific model as a package contributor.
...
can use small in-memory metadata data frames. You do not need a
recodeflow package or separate CSV files for a one-off example.
