...
Generate realistic test data from `recodeflow` variable configuration
files (`variables.csv` and `variable-details.csv`).
...
just variable names, types, and ranges. Total 9 rows across 2 files.
...
2.  **New to metadata?** Upfront cost to create configuration files, but
    you get reproducible data pipelines and documentation that stays in
    sync with your code.
...
- **Metadata-driven**: Uses existing `variables.csv` and
  `variable_details.csv` from recodeflow projects
...
- [Survival
  data](https://big-life-lab.github.io/MockData/vignettes/survival-data.qmd) -
